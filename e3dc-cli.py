...
def ParseConfig():
  ...
  argparser.add_argument(
      "--connection.address", type=str, help="IP or DNS address of the E3/DC system"
  )
  argparser.add_argument(
      "--connection.user",
      type=SecretStr,
  ...
  argparser.add_argument(
      ...
      choices=[
          ...
          # History queries
          QueryType.history_today.name,
          QueryType.history_yesterday.name,
          QueryType.history_week.name,
          QueryType.history_previous_week.name,
          QueryType.history_month.name,
          QueryType.history_previous_month.name,
          QueryType.history_year.name,
          QueryType.history_previous_year.name,
...
def RunMultiQuery(e3dc, query_config):
  ...
  for query in query_config:
    ...
    if single_query_result is not None:
      collected_data[query] = single_query_result
...
def RunSingleQuery(e3dc, query):
  ...
  if query == QueryType.live.name:
    result = e3dc.poll(keepAlive=True)
  elif query == QueryType.live_system.name:
    sys_status = e3dc.get_system_status(keepAlive=True)
    ...
    result = e3dc.get_pvi_data(keepAlive=True)
  elif query == QueryType.live_wallbox.name:
    result = e3dc.get_wallbox_data(keepAlive=True)
...
def QueryHistoryDatabase(e3dc, startTimestamp, timespanSeconds):
  return e3dc.get_db_data_timestamp(
      startTimestamp=startTimestamp, timespanSeconds=timespanSeconds, keepAlive=True
  )
